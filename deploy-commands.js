...
try {
    ...
    const commandsToDelete = existingCommands.filter(command => !commands.some(cmd => cmd.name === command.name));
    if (commandsToDelete.length > 0) console.log('\nThe following unused commands will be deleted:', commandsToDelete.map(cmd => `- /${cmd.name}`).join('\n'), '\n');
    ...
    for (const command of existingCommands) {
        ...
        try {
            ...
            if (error.code === 10063) console.log(`Command ${command.name} already deleted or doesn't exist`);
            else console.error(`Failed to delete command ${command.name}: ${error.message}`);
        }
    }
